...
void CompareSelectedEvents() {
  ...
  // Arrays for tracking selected entries
  std::vector<int> OAGW_SelectedEntryNum;
  std::vector<int> HL_SelectedEntryNum;

  // OAGW entry selection
  for (uint i=0; i<sample_sum->GetEntries(); i++) {
    ...
    std::cout << "OAGW entry " << i << " selected" << std::endl;
    OAGW_SelectedEntryNum.push_back(i);
  }
  ...
  for (uint i=0; i<anaTree->GetEntries(); i++) {
    ...
    std::cout << "HL entry " << i << " selected" << std::endl;
    HL_SelectedEntryNum.push_back(i);
  }
  ...
  // Loop over selected OAGW entries
  for (uint i=0; i<=OAGW_SelectedEntryNum.size(); i++) {

    int entry = OAGW_SelectedEntryNum[i];

    sample_sum->GetEntry(entry);

    // Try to find the same entry in the selected HL events
    bool matched = false;

    for (uint j=0; j<=HL_SelectedEntryNum.size(); j++) {
      // Search outward from the selected entry number
      int high = HL_SelectedEntryNum[i+j];
      int low = HL_SelectedEntryNum[i-j];
      ...
      if ( high < anaTree->GetEntries()) {
        ...
        if ( (AT_evt == SS_EventNumber) && (AT_bunch == FT_Bunch) ) {
          std::cout << "Found OAGW entry " << entry << " in the HL file (" << high << ")" << std::endl;
          matched = true;
          break;
        }
      }
      ...
      if ( (low >= 0) && (high != low) ) {
        ...
        if ( (AT_evt == SS_EventNumber) && (AT_bunch == FT_Bunch) ) {
          std::cout << "Found OAGW entry " << entry << " in the HL file (" << low << ")" << std::endl;
          matched = true;
          break;
        }
      }
    ...
    if (!matched) {
      std::cout << "Didn't find OAGW entry " << entry << " in the HL file" << std::endl;
      eventPrint << "Entry=" << entry << ", Spill=" << SS_EventNumber << ", Bunch=" << FT_Bunch << ", Sample=" << SS_SelectedSample << std::endl;
    }
  }
  ...
  // Loop over selected HL entries
  for (uint i=0; i<=HL_SelectedEntryNum.size(); i++) {

    int entry = HL_SelectedEntryNum[i];

    anaTree->GetEntry(entry);

    // Try to find the same entry in the selected OAGW events
    bool matched = false;

    for (uint j=0; j<=OAGW_SelectedEntryNum.size(); j++) {
      // Search outward from the event number
      int high = OAGW_SelectedEntryNum[i+j];
      int low = OAGW_SelectedEntryNum[i-j];
      if ( (low < 0) && (high >= sample_sum->GetEntries()) ) break;

      if ( high < sample_sum->GetEntries()) {
        sample_sum->GetEntry(high);
        if ( (AT_evt == SS_EventNumber) && (AT_bunch == FT_Bunch) ) {
          std::cout << "Found HL entry " << entry << " in the OAGW file (" << high << ")" << std::endl;
          matched = true;
          break;
        }
      }
      ...
      if ( (low >= 0) && (high != low) ) {
        ...
        if ( (AT_evt == SS_EventNumber) && (AT_bunch == FT_Bunch) ) {
          std::cout << "Found HL entry " << entry << " in the OAGW file (" << low << ")" << std::endl;
          matched = true;
          break;
        }
      }
    ...
    // If we didn't find it, print it
    if (!matched) {
      std::cout << "Didn't find HL entry " << entry << " in the OAGW file" << std::endl;
      eventPrint << "Entry=" << entry << ", Spill=" << AT_evt << ", Bunch=" << AT_bunch << ", Sample=" << AT_sample << std::endl;
    }
  }
